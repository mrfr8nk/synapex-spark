...
const AdminCertifications = () => {
  ...
  const handleSave = async () => {
    const payload = {
      ...
      isVisible: form.is_visible,
      issueDate: form.issue_date,
      credentialId: form.credential_id,
      credentialUrl: form.credential_url,
      imageUrl: form.image_url,
      skills: skillsInput.split(",").map((s: string) => s.trim()).filter(Boolean),
    };
    try {
      if (editId === "new") await api.createCertification(payload);
      else await api.updateCertification(editId!, payload);
  ...
  const toggleVisible = async (c: any) => {
    await api.updateCertification(c.id, { isVisible: !c.is_visible });
    qc.invalidateQueries({ queryKey: ["admin_certifications"] });
    qc.invalidateQueries({ queryKey: ["certifications"] });
  };
