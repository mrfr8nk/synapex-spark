...
router.post("/projects", requireAuth, async (req, res) => {
  const [row] = await db.insert(projectsTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
...
router.post("/milestones", requireAuth, async (req, res) => {
  const [row] = await db.insert(milestonesTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
...
router.post("/currently-building", requireAuth, async (req, res) => {
  const [row] = await db.insert(currentlyBuildingTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
...
router.post("/social-links", requireAuth, async (req, res) => {
  const [row] = await db.insert(socialLinksTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
...
router.post("/what-i-build", requireAuth, async (req, res) => {
  const [row] = await db.insert(whatIBuildTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/what-i-build/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(whatIBuildTable).set({ ...req.body, updatedAt: new Date() }).where(eq(whatIBuildTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.post("/footer-links", requireAuth, async (req, res) => {
  const [row] = await db.insert(footerLinksTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/footer-links/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(footerLinksTable).set({ ...req.body, updatedAt: new Date() }).where(eq(footerLinksTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.post("/blog-posts", requireAuth, async (req, res) => {
  const [row] = await db.insert(blogPostsTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/blog-posts/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(blogPostsTable).set({ ...req.body, updatedAt: new Date() }).where(eq(blogPostsTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.get("/skills", async (_req, res) => {
  const rows = await db.select().from(skillsTable).orderBy(asc(skillsTable.category), asc(skillsTable.sortOrder));
  res.json(rows);
});
router.post("/skills", requireAuth, async (req, res) => {
  const [row] = await db.insert(skillsTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/skills/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(skillsTable).set({ ...req.body, updatedAt: new Date() }).where(eq(skillsTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.post("/testimonials", requireAuth, async (req, res) => {
  const [row] = await db.insert(testimonialsTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/testimonials/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(testimonialsTable).set({ ...req.body, updatedAt: new Date() }).where(eq(testimonialsTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.post("/certifications", requireAuth, async (req, res) => {
  const [row] = await db.insert(certificationsTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/certifications/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(certificationsTable).set({ ...req.body, updatedAt: new Date() }).where(eq(certificationsTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.post("/education", requireAuth, async (req, res) => {
  const [row] = await db.insert(educationTable).values({ ...req.body, id: crypto.randomUUID() }).returning();
  res.json(row);
});
router.put("/education/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(educationTable).set({ ...req.body, updatedAt: new Date() }).where(eq(educationTable.id, req.params.id)).returning();
  res.json(row);
});
...
router.post("/contact-messages", async (req, res) => {
  ...
  const [row] = await db.insert(contactMessagesTable).values({ id: crypto.randomUUID(), name, email, message }).returning();
  res.json(row);
});
router.put("/contact-messages/:id", requireAuth, async (req, res) => {
  const [row] = await db.update(contactMessagesTable).set(req.body).where(eq(contactMessagesTable.id, req.params.id)).returning();
  res.json(row);
});
