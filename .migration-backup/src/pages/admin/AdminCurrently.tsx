...
const AdminCurrently = () => {
  ...
  const handleSave = async () => {
    ...
    if (editId === "new") {
      await supabase.from("currently_building").insert(payload);
    } else {
      await supabase.from("currently_building").update(payload).eq("id", editId);
    }
  ...
  const handleDelete = async (id: string) => {
    await supabase.from("currently_building").delete().eq("id", id);
    queryClient.invalidateQueries({ queryKey: ["currently_building"] });
  };
